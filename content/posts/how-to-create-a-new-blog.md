...
**部署机制详解**：
1.  代码被推送到 GitHub 远程仓库。
2.  Vercel 监听到 Webhook 事件，自动拉取最新提交。
3.  Vercel 云端环境执行 `hugo` 构建命令，生成静态 HTML 产物。
4.  构建产物被即时分发至全球边缘网络（Edge Network）。
...
*   **构建行为**：构建时，`static/` 目录下的内容会被直接拷贝至网站输出根目录（public/），且**保持原有目录层级**。
*   **引用规范**：在 Markdown 中引用资源时，路径应相对于根目录，**严禁**包含 `static` 前缀。
